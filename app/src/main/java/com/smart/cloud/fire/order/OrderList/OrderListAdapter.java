...
public class OrderListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof ItemViewHolder) {
            ...
            ((ItemViewHolder) holder).content_tv.setText("详情:"+mJobOrder.getDescription());
            ((ItemViewHolder) holder).user_tv.setText(mJobOrder.getDealUserName());
            String state="";
            switch (mJobOrder.getState()){
                case 1:
                    state="未派单";
                    ((ItemViewHolder) holder).deal_state_im.setImageResource(R.drawable.order_paidan);
                    ((ItemViewHolder) holder).deal_state_tv.setText("派发");
                    break;
                case 2:
                    state="未接单(点击接单)";
                    ((ItemViewHolder) holder).deal_state_im.setImageResource(R.drawable.order_chuli);
                    ((ItemViewHolder) holder).deal_state_tv.setText("处理");
                    break;
                case 3:
                    state="待复核";
                    ((ItemViewHolder) holder).deal_state_im.setImageResource(R.drawable.order_chuli);
                ...
                case 4:
                    state="已处理";
                    ((ItemViewHolder) holder).deal_state_im.setImageResource(R.drawable.order_xiangqing);
                    ((ItemViewHolder) holder).deal_state_tv.setText("详情");
                    break;
                case 5:
                    state="上报审核";
                    ((ItemViewHolder) holder).deal_state_im.setImageResource(R.drawable.order_chuli);
    ...
    public static class ItemViewHolder extends RecyclerView.ViewHolder {
        ...
        @Bind(R.id.time_tv)
        TextView time_tv;
        @Bind(R.id.deal_state_im)
        ImageView deal_state_im;
        @Bind(R.id.deal_state_tv)
