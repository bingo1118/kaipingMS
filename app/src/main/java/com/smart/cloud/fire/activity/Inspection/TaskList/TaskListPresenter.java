...
public class TaskListPresenter extends BasePresenter<TaskListView>{
    ...
    public void getTasksNum(String userId){
        mvpView.showLoading();
        Observable mObservable=apiStores1.getTaskCount(userId);
        addSubscription(mObservable,new SubscriberCallBack<>(new ApiCallback<HttpError>() {
            @Override
            public void onSuccess(HttpError model) {
                mvpView.getSumDataSuccess(model.getTodoNum(),model.getProgreaNum(),model.getComplishNum());
            }
            @Override
            public void onFailure(int code, String msg) {
                mvpView.getDataFail("网络错误");
